...
func createMigration(name string) string {
	...
	if err != nil {
		logger.Fatal(fmt.Sprintf(`Something went wrong while try to parse template file!`, err))
	}

	file, err := os.Create(fmt.Sprintf(`./data/migrations/postgres/%s.go`, cleanedMigrationName))
	if err != nil {
		logger.Fatal(fmt.Sprintf(`Something went wrong while try to create migration file!`, err))
	}

	defer file.Close()

	if _, err := file.WriteString(fileBuffer.String()); err != nil {
		logger.Fatal(fmt.Sprintf(`Something went wrong while try to write migration file!`, err))
	}
...
var CreateMigrationCommand = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			logger.Fatal(`Something went wrong while claiming requeired "name" argument`)
			return
		}

		migrationName := createMigration(name)

		logger.Info(fmt.Sprintf(`Migration %s created successfully`, migrationName))
	},
}
